...
def _keyword_pattern(kw: str) -> "re.Pattern":
    ...
    if pattern is None:
        ...
        # which silently mistagged huge swaths of unrelated questions. Use
        # word boundaries, but only where the keyword's edge is actually
        # alphanumeric (symbols/percent signs aren't word characters, so \b
        # around them is moot).
...
def infer_topic_for_page(page_text: str, source_display: str, folder: str) -> Dict[str, str]:
    """Multi-topic test papers have their filename/folder bleed into every
    question's classification, sometimes overriding a perfectly good
    content-based match. Trust the actual page content first; only fall
    back to filename/folder context for pages with no recognisable
    vocabulary at all (e.g. a lone diagram-based question)."""
